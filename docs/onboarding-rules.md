...
4. **Register** the rule: either in a bundle used by a preset ([`bundle.rs`](../crates/rules/src/bundle.rs), [`preset.rs`](../crates/rules/src/preset.rs), `Registry::v0_default()` / `home_default()` / `minimal_default()`), or via `Registry::from_rules` for a custom registry. Adapters loading `rusthome.toml` should use [`rusthome_app::rusthome_file`](../crates/app/src/rusthome_file.rs) (same as the CLI).
