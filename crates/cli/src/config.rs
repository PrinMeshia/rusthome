//! Optional `{data-dir}/rusthome.toml` — rules preset + runtime parameters (`ConfigSnapshot`).
...
pub struct RusthomeFile {
    ...
    /// `simulation` or `io_anchored` (snake_case). Used when the subcommand does not pass `--io-anchored`.
    #[serde(default)]
...
pub fn validate_rusthome_file(file: &RusthomeFile) -> Result<(), String> {
    ...
    if let Some(d) = file.io_timeout_logical_delta {
        if d <= 0 {
            return Err(format!(
                "io_timeout_logical_delta: must be > 0, got {d}"
            ));
        }
    }
...
/// Merges `[run_limits]` with `rusthome_app::RunLimits` defaults (§6.6).
...
/// `--io-anchored` forces [`PhysicalProjectionMode::IoAnchored`]; else file then default simulation.
...
/// Order: `--rules-preset` then `RUSTHOME_RULES_PRESET` (clap) then `rusthome.toml` then `v0`.
...
"#,
        ...
        assert_eq!(
            ...
            rusthome_app::RunLimits::default().max_events_generated_per_root
        );
    }
