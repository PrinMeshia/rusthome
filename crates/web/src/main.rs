//! Read-only web dashboard — replay journal → `State`, optional JSON APIs.
//!
//! **Dev / lab only**: binds to `127.0.0.1` by default, no authentication.
...
#[command(about = "Minimal read-only rusthome dashboard (replay journal)")]
struct Args {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    eprintln!(
        "rusthome-web listening on http://{} (data-dir={})",
        args.bind,
